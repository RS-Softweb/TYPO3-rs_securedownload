...
class tx_rssecuredownload_pi1 extends AbstractPlugin
{
    ...
    public $scriptRelPath = 'pi1/class.tx_rssecuredownload_pi1.php';
        ...
        switch ($action) {
            case 'checkCode':
                if (($tryall == 1) && ($this->piVars['download'] == 0)) {
                    $code_query = $db->exec_SELECTquery(
                        '*',
                        'tx_rssecuredownload_codes',
                        'code="' . addslashes($givenCode) . '"'
                    );
                } else {
                    $code_query = $db->exec_SELECTquery(
                        '*',
                        'tx_rssecuredownload_codes',
                        'uid="' . addslashes($downloadid) . '" AND code="' . addslashes($givenCode) . '"'
                    );
                }

                if (($tryall == 0) && ($this->piVars['download'] != $downloadid)) {
                    break;
                }
                ...
                $query_local = $db->exec_SELECTquery(
                    '*',
                    'tx_rssecuredownload_codes',
                    'uid="' . addslashes($downloadid) . '"'
                );
