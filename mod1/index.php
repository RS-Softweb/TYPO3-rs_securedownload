...
//				$codes_query = $db->exec_SELECTquery('*', 'tx_rssecuredownload_codes', 'pid IN ('.$this->pagelist.')');
    ...
    private function markDeleted()
    {
        ...
        switch ($this->deleteType) {
            case 3:
                $update_result = $db->exec_UPDATEquery('tx_rssecuredownload_logs', 'uid=' . $this->deleteCode, ['deleted'=>'1']);
            break;
            case 2:
                $update_result = $db->exec_UPDATEquery('tx_rssecuredownload_logs', 'docid=' . $this->deleteCode . ' AND error=' . '0', ['deleted'=>'1']);
            break;
            case 1:
                $update_result = $db->exec_UPDATEquery('tx_rssecuredownload_logs', 'docid=' . $this->deleteCode . ' AND NOT error=' . '0', ['deleted'=>'1']);
            break;
            case 0:
                $update_result = $db->exec_UPDATEquery('tx_rssecuredownload_logs', 'docid=' . $this->deleteCode, ['deleted'=>'1']);
            break;
        }
    }
